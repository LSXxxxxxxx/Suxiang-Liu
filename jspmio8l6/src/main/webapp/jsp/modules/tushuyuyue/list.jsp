...
<body>
	<div id="main-container">
		...
		<div id="breadcrumb-container">
			<h3 class="breadcrumb-title">图书预约管理</h3>
			<ol class="breadcrumb-list">
				...
				<li class="breadcrumb-item-one"><span>图书预约管理</span></li>
				<li class="breadcrumb-item-two"><span>图书预约列表</span></li>
			</ol>
		</div>
		<!-- /Breadcrumb -->
		...
		<div id="center-container">
			...
			<div class="center-form">
				<div class="form-item">
					...
					<input type="text" id="tiaoxingmaSearch" class="form-control form-control-sm" placeholder="请输入条形码" aria-controls="tableId">
				</div>
				<div class="form-item">
					<label>图书名称</label>
	...
	<script>
		...
		function search(){
			searchForm = { key: ""};
			if($('#tiaoxingmaSearch').val() != null && $('#tiaoxingmaSearch').val() != ''){
				searchForm.tiaoxingma ="%" +  $('#tiaoxingmaSearch').val() + "%";
			}
        ...
        function getDataList() {
			http("tushuyuyue/page","GET",{
				page: pageIndex,
				limit: pageSize,
                sfsh: searchForm.sfsh,
				sort: sortColumn,
				order: sortOrder,
				tiaoxingma : searchForm.tiaoxingma,
		...
		function shDialog(id){
			$("#shId").attr("value",id);
			http("tushuyuyue/info/" + id,"GET",{},(res)=>{
				if(res.code == 0){
		...
		function sh(){
			...
			httpJson("tushuyuyue/update","POST",data,(res)=>{
				if(res.code == 0){
					alert("提交成功");
					getDataList();
				}
        ...
        function remove(id) {
            ...
            if (mymessage == true) {
				...
				httpJson("tushuyuyue/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();
						alert('删除成功');
					}
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"tushuyuyue");
